...
public class MbAnalysis {
    ...
    private static ArrayList<Rule> associationRules = new ArrayList<Rule>();
    ...
    public MbAnalysis() {
        ...
        //MbAnalysis newobj=new MbAnalysis(); 
        getDatabaseInput();
    }
    ...
    public static void getDatabaseInput() {
        ...
        try {
            ...
            rs = new DatabaseConnect().stmt.executeQuery("SELECT count(distinct(VCHRNO))from sales");//set date from outside
            rs.next();

            noOftransaction = rs.getInt(1);//for support calculation

            //  rs = new DatabaseConnect().stmt.executeQuery("SELECT classkey, vchrno FROM `sales` where date between '"+fd+"' and '"+td+"'  ");//y not done sorting here
            rs = new DatabaseConnect().stmt.executeQuery("SELECT classkey, vchrno FROM `sales`  order by classkey asc, vchrno asc");
            ...
            Iterator<Rule> iterator = associationRules.iterator();
           while (iterator.hasNext())
           {
           Rule r1 = iterator.next();
           System.out.println("Prinitng new Rule");
           System.out.println(r1.getLeft());
           System.out.println(">>");
           System.out.println(r1.getRight());
           System.out.println("*************");
    ...
    private static void assoRulesGen() {
        ...
        for (int i = 0; i < associationRules.size(); i++) {
            for (int k = 0; k < ((Rule) associationRules.get(i)).getLeft().size(); k++) {
             ...
             //   System.out.println(associationRules.get(i).getLeft());
            }
            ...
            for (int k = 0; k < ((Rule) associationRules.get(i)).getRight().size(); k++) {
                ...
                (associationRules.get(i)).getRight().set(k, cList.get(r - 1));
                 //    System.out.println(associationRules.get(k).getRight());
            }
        }
    }
